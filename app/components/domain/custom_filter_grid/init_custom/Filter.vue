...
const find = async () => {
  ...
  try {
    ...
    active.value = res.rows
      .filter((e) => e.initial_filter == true && e.active == true)
      .sort((a, b) => a.sequence_filter! - b.sequence_filter!);

    deactivated.value = res.rows.filter(
      (e) => e.initial_filter == false && e.active == true
    );
...
const save = async () => {
  ...
  try {
    ...
    $toast().success("Filtros iniciais salvo com sucesso");
...
<template>
  ...
  >
    ...
    <template #default>
      <v-card title="FILTROS INICIAIS" rounded="xl">
        <template #text>
          <v-row dense>
            <v-col cols="8">
